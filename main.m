...
v_TAS = zeros(input_quantity,1);
k_p = zeros(input_quantity,1);
k_lag = zeros(input_quantity,1);
c = zeros(input_quantity,1);

%Preallokation der Structs
BFZ(1:input_quantity) = struct();
AS(1:input_quantity) = struct();
BS(1:input_quantity) = struct();
DGS(1:input_quantity) = struct();
NickD(1:input_quantity) = struct();

i=0; % Initialisierung der Laufvariablen
for m = 1:length(input_height)
    for n = 1:length(input_speed)
        i = i+1;
        %% --- Bezugsflugzustand ---
        ...
        %% Abspeichern der Trimmgrößen für Lookup-Table
        H_ft(i) = input_height(m);
        H_m(i) = BFZ(i).h;
        v_kt(i) = input_speed(n);
        v_TAS(i) = BFZ(i).V;

        k_p(i) = NickD(i).k_p;
        k_lag(i) = NickD(i).Lag.k;
        c(i) = NickD(i).Lag.c;
    end
end
